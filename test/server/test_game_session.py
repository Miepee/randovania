...
def test_create_game_session(clean_database, preset_manager):
    ...
    assert convert_to_raw_python(BinaryMultiplayerSessionEntry.parse(result)) == {
        'id': 1,
        'state': MultiplayerSessionState.SETUP.value,
        'name': 'My Room',
...
def test_join_game_session(mock_emit_session_update: MagicMock,
    ...
    database.World.create(session=session, row=0, preset="{}")
...
def test_game_session_request_pickups_not_in_game(flask_app, clean_database):
    ...
    membership = database.MultiplayerMembership.create(user=user, session=session, row=0, admin=False)
    ...
    with pytest.raises(RuntimeError, match="Unable to emit pickups during SETUP"):
        game_session.emit_game_session_pickups_update(sio, membership)


def test_game_session_request_pickups_observer(flask_app, clean_database):
    # Setup
    user = database.User.create(id=1234, discord_id=5678, name="The Name")
    session = database.MultiplayerSession.create(name="Debug", creator=user, state=MultiplayerSessionState.IN_PROGRESS)
    membership = database.MultiplayerMembership.create(user=user, session=session, row=None, admin=False)
...
def two_player_session_fixture(clean_database):
    ...
    session = database.MultiplayerSession.create(id=1, name="Debug", state=MultiplayerSessionState.IN_PROGRESS, creator=user1)
    database.World.create(session=session, row=0, preset="{}")
    database.World.create(session=session, row=1, preset="{}")

    database.MultiplayerMembership.create(user=user1, session=session, row=0, admin=False)
    database.MultiplayerMembership.create(user=user2, session=session, row=1, admin=False)
    database.WorldAction.create(session=session, provider_row=1, provider_location_index=0, receiver_row=0)
...
def test_game_session_request_pickups_one_action(mock_session_description: PropertyMock,
    ...
    membership = database.MultiplayerMembership.get(user=database.User.get_by_id(1234), session=two_player_session)
...
def test_game_session_collect_pickup_for_self(mock_session_description: PropertyMock,
    ...
    with pytest.raises(peewee.DoesNotExist):
        database.WorldAction.get(session=two_player_session, provider_row=0,
...
def test_game_session_collect_pickup_other(flask_app, two_player_session, echoes_resource_database,
    ...
    membership = database.MultiplayerMembership.get_by_session_position(two_player_session, row=1)

    for existing_id in exists:
        database.WorldAction.create(session=two_player_session, provider_row=0,
...
def test_game_session_admin_player_switch_is_observer(clean_database, flask_app, mock_emit_session_update, is_observer):
    ...
    session = database.MultiplayerSession.create(id=1, name="Debug", state=MultiplayerSessionState.IN_PROGRESS, creator=user1)
    database.World.create(session=session, row=0, preset="{}")
    database.MultiplayerMembership.create(user=user1, session=session, row=None if is_observer else 0, admin=False)
    sio = MagicMock()
    sio.get_current_user.return_value = user1
...
def test_game_session_admin_player_include_in_session(clean_database, flask_app, mock_emit_session_update):
    ...
    session = database.MultiplayerSession.create(id=1, name="Debug", state=MultiplayerSessionState.IN_PROGRESS, creator=users[0])
    for i in range(4):
        database.World.create(session=session, row=i, preset="{}")

    database.MultiplayerMembership.create(user=users[0], session=session, row=0, admin=True)
    database.MultiplayerMembership.create(user=users[1], session=session, row=2, admin=False)
    database.MultiplayerMembership.create(user=users[2], session=session, row=1, admin=False)
    database.MultiplayerMembership.create(user=users[3], session=session, row=None, admin=False)
    sio = MagicMock()
...
def test_game_session_admin_kick_last(clean_database, flask_app, mocker, mock_audit):
    ...
    database.WorldAction.create(session=session, provider_row=0, provider_location_index=0, receiver_row=0,
    ...
    mock_emit.assert_called_once_with(
        ...
        BinaryMultiplayerSessionEntry.build({'id': 1, 'name': 'My Room', 'state': 'setup', 'players': [], 'presets': [],
...
def test_game_session_admin_player_move(clean_database, flask_app, mock_emit_session_update, offset: int):
    ...
    database.World.create(session=session, row=0, preset="{}")
    database.World.create(session=session, row=1, preset="{}")
    database.World.create(session=session, row=2, preset="{}")
    database.MultiplayerMembership.create(user=user1, session=session, row=1, admin=False)
    sio = MagicMock()
    sio.get_current_user.return_value = user1
...
def test_game_session_admin_player_patcher_file(mock_layout_description: PropertyMock,
    ...
    database.World.create(session=session, row=1, preset="{}")
    database.World.create(session=session, row=2, preset="{}")
    database.MultiplayerMembership.create(user=user1, session=session, row=2, admin=False)
    database.MultiplayerMembership.create(user=user2, session=session, row=1, admin=False)
    sio = MagicMock()
    sio.get_current_user.return_value = user1
...
def test_game_session_admin_session_create_row(mock_emit_session_update: MagicMock,
    ...
    # Assert
    mock_emit_session_update.assert_called_once_with(session)
    assert database.MultiplayerSession.get_by_id(1).num_rows == 1
...
def test_game_session_admin_session_change_row(mock_emit_session_update: MagicMock,
    ...
    # Assert
    mock_emit_session_update.assert_called_once_with(session)
    new_preset_row = database.World.get(database.World.session == session,
                                        database.World.row == 1)
...
def test_game_session_admin_session_delete_row_invalid(mock_emit_session_update,
    ...
    mock_emit_session_update.assert_not_called()
    assert database.MultiplayerSession.get_by_id(1).num_rows == expected_num_rows
...
def test_game_session_admin_session_change_layout_description(clean_database, preset_manager, mock_emit_session_update,
    ...
    database.World.create(session=session, row=0, preset=preset_as_json)
    database.World.create(session=session, row=1, preset=preset_as_json)
    database.MultiplayerMembership.create(user=user1, session=session, row=None, admin=True)
...
def test_game_session_admin_session_download_layout_description(mock_layout_description: PropertyMock, flask_app,
    ...
    session = database.MultiplayerSession.create(id=1, name="Debug", state=MultiplayerSessionState.SETUP, creator=user1,
                                                 layout_description_json="layout_description_json")
    database.MultiplayerMembership.create(user=user1, session=session, row=None, admin=False)
    sio = MagicMock()
    sio.get_current_user.return_value = user1
...
def test_game_session_admin_session_duplicate_session(clean_database, mock_emit_session_update, flask_app, mock_audit):
    ...
    session = database.MultiplayerSession.create(id=1, name="Debug", state=MultiplayerSessionState.SETUP, creator=user1)
    database.World.create(session=session, row=0, preset="{}")
    database.World.create(session=session, row=1, preset='{"foo": 5}')
    database.MultiplayerMembership.create(user=user1, session=session, row=0, admin=True)
    database.MultiplayerMembership.create(user=user2, session=session, row=1, admin=True)
    sio = MagicMock()
    sio.get_current_user.return_value = user1
...
def test_emit_session_meta_update(session_update, flask_app, mocker):
    ...
    # Run
    with flask_app.test_request_context():
        randovania.server.multiplayer.session_common.emit_session_meta_update(session_update)

    # Assert
    mock_emit.assert_called_once_with(
        "game_session_meta_update",
        BinaryMultiplayerSessionEntry.build(session_json),
...
def test_emit_session_actions_update(session_update, flask_app, mocker):
    ...
    # Run
    with flask_app.test_request_context():
        randovania.server.multiplayer.session_common.emit_session_actions_update(session_update)
...
def test_emit_session_audit_update(session_update, flask_app, mocker):
    ...
    # Run
    with flask_app.test_request_context():
        randovania.server.multiplayer.session_common.emit_session_audit_update(session_update)
...
def test_game_session_request_update(session_update, mocker, flask_app):
    ...
    sio = MagicMock()
    sio.get_current_user.return_value = user
    membership = database.MultiplayerMembership.get(user=user, session=session_update)
